...
func runMigrations(ctx context.Context, pool *pgxpool.Pool, dir string) error {
	...
	for _, f := range files {
		...
		upSQL := extractGooseUp(rawContent)
		if upSQL == "" {
			continue
		}
		// Strip CONCURRENTLY for test containers — pgxpool.Exec uses
		// implicit transactions which block CONCURRENTLY operations.
		if strings.Contains(rawContent, "-- +goose NO TRANSACTION") {
...
// openStdlibDB creates a database/sql.DB from the superuser pool's connection config.
// Watermill requires *sql.DB, not pgxpool.
...
func TestAuditSubscriber_PersistsEvent(t *testing.T) {
	...
	if storedType != "endpoint.created" {
		t.Errorf("type = %q, want %q", storedType, "endpoint.created")
	}
	if storedActorID != "user-1" {
		t.Errorf("actor_id = %q, want %q", storedActorID, "user-1")
	}
	if storedResource != "endpoint" {
		t.Errorf("resource = %q, want %q", storedResource, "endpoint")
	}
}
...
func TestAuditSubscriber_TenantIsolation(t *testing.T) {
	...
	evtA := domain.NewAuditEvent("endpoint.created", tenantA, "user-a", domain.ActorUser, "endpoint", "ep-a", "created", nil, domain.EventMeta{})
	evtB := domain.NewAuditEvent("endpoint.created", tenantB, "user-b", domain.ActorUser, "endpoint", "ep-b", "created", nil, domain.EventMeta{})
...
func TestWildcardPatternMatching(t *testing.T) {
	tests := []struct {
		...
		{"deployment prefix", "deployment.*", 13},
		{"endpoint prefix", "endpoint.*", 8},
		{"exact match", "policy.created", 1},
